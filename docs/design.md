...
- **Art.** Engravings are generated one piece at a time (not cropped from the reference) and live in `public/art/*.webp`. Scene art has its paper colour keyed out to transparency; the three passage thumbnails stay rectangular. All of it is decoration only (`aria-hidden`, `pointer-events: none`) and is hidden below `lg`, except the hero cliff, which stacks under the headline on small screens. The hero's strata labels are HTML over the image, not part of it. Handwritten notes use Caveat (`t-hand`), also decoration only, and never carry information that is not also in the body copy.
